...
RUN wget -q -O /tmp/misspell-install.sh https://raw.githubusercontent.com/golangci/misspell/master/install-misspell.sh \
    && sh /tmp/misspell-install.sh -b /usr/local/bin/ "${MISSPELL_VERSION}" \
    && rm /tmp/misspell-install.sh
